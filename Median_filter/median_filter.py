...
if __name__ == '__main__':
    ...
    #import image:
    data=sio.loadmat('aligned_xyzt_data.mat')
    nm_img_stack=data['nm_img_stack']
    nm_img_stack_filtered=np.zeros_like(nm_img_stack)
    for z in range(nm_img_stack.shape[2]):
        for t in range(nm_img_stack.shape[3]):

            host_image=nm_img_stack[:,:,z,t].astype(np.float32).copy()
            ...
            nm_img_stack_filtered[:,:,z,t]=host_image_filtered

    im=plt.imshow(nm_img_stack_filtered[:,:,10,:])
    sio.savemat('xyzt_filtered_data.mat',{'nm_img_stack_filtered':nm_img_stack_filtered})